...
def spremi_u_csv(self):
    if not self.ucenici:
        messagebox.showinfo("Nema podataka", "Nema učenika za spremiti.")
        return
    filepath = filedaialog.asksaveasfilename(defaultextension=".csv",filetypes=[("CSV files", "*.csv"))
     if filepath:
        ...
        messagebox.showinfo("Spremanje uspješno", f"Podaci su spremljeni u {filepath}")
def ucitaj_iz_csv(self):
    filepath = filedaialog.askopenfilename(filetypes=[("CSV", "*.csv")])
    if filepath:
        with open(filepath, mode='r', newline='', encoding='utf-8') as file:
            reader = csv.DictReader(file)
            self.ucenici = [Ucenik(row["Ime"], row["Prezime"], row["Razred"]) for row in reader]
        self.azuriraj_listbox()
        messagebox.showinfo("Učitavanje uspješno", f"Podaci su učitani iz {filepath}")
